            ...
            <div style={{ all: 'initial', boxSizing: 'border-box' }} className="pointer-events-auto">
                <div
                    className={`bg-gray-900 shadow-[0_20px_60px_rgba(0,0,0,0.6)] border-2 flex ${isVertical ? 'flex-col' : 'flex-row'} items-center box-border text-white whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${settings.isCleanView ? 'opacity-40 hover:opacity-100' : ''} ${isVisible
                        ? (isVertical ? 'w-15 max-h-[98vh] h-fit rounded-[22px] p-2 px-1 gap-1' : 'w-fit max-w-[98vw] h-15 rounded-[22px] p-1 px-2 gap-1')
                        : 'w-13 h-13 rounded-xl p-0 gap-0 border-0'
                        }`}
                    style={{
                ...
                >
                    {/* 1. 확장 상태 도구 레이어 (Expanded Tools Layer) */}
                    <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} items-center gap-1.5 transition-all duration-500 ${isVisible ? 'opacity-100' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                        {isExpanded && (
                            <>
                                {/* 상단 닫기 (X) 버튼 - 확장 상태 */}
                                <button
                                ...
                                >
                                    {settings.isCleanView ? <EyeOff size={18} /> : <Eye size={18} />}
                                </button>
                                ...
                                {mode === 'markup' && (
                                    <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} gap-1 items-center`}>
                                        <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} bg-white/5 rounded-xl p-1 border border-white/5 items-center`}>
                                            <button onClick={() => setTool('pen')} className={`p-1 rounded-lg transition-all ${currentTool === 'pen' ? 'text-gray-900' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} style={currentTool === 'pen' ? { backgroundColor: accentColor } : {}} title="펜"><PenTool size={16} /></button>
                                            <button onClick={() => setTool('highlight')} className={`p-1 rounded-lg transition-all ${currentTool === 'highlight' ? 'text-gray-900' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} style={currentTool === 'highlight' ? { backgroundColor: accentColor } : {}} title="형광펜"><Highlighter size={16} /></button>
                                            <button onClick={() => setTool('eraser')} className={`p-1 rounded-lg transition-all ${currentTool === 'eraser' ? 'bg-red-500 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} title="지우개"><Eraser size={16} /></button>
                                            <button onClick={() => setTool('select')} className={`p-1 rounded-lg transition-all ${currentTool === 'select' ? 'text-gray-900' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} style={currentTool === 'select' ? { backgroundColor: accentColor } : {}} title="선택 및 수정"><MousePointer2 size={16} /></button>
                                        </div>

                                        <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} bg-white/5 rounded-xl p-1 border border-white/5 items-center gap-0.5`}>
                                            <div className="relative">
                                                <button onClick={() => setIsShapePickerOpen(!isShapePickerOpen)} className={`p-1 rounded-lg transition-all ${['rect', 'circle', 'arrow', 'star', 'heart', 'triangle', 'chat', 'lightning', 'diamond', 'pentagon', 'hexagon', 'cross', 'cloud', 'banner'].includes(currentTool) ? 'text-gray-900' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} style={['rect', 'circle', 'arrow', 'star', 'heart', 'triangle', 'chat', 'lightning', 'diamond', 'pentagon', 'hexagon', 'cross', 'cloud', 'banner'].includes(currentTool) ? { backgroundColor: accentColor } : {}} title="도형">{shapes.find(s => s.id === selectedShape)?.icon || <Square size={16} />}</button>
                                                {isShapePickerOpen && (
                                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 bg-gray-800 shadow-2xl border border-white/10 rounded-2xl p-2 grid grid-cols-4 gap-1 backdrop-blur-xl z-[100] w-48">
                                                        {shapes.map(s => (<button key={s.id} onClick={() => { setSelectedShape(s.id); setTool(s.id); setIsShapePickerOpen(false); }} className={`p-2 rounded-xl transition-all ${currentTool === s.id ? 'text-gray-900' : 'text-gray-400 hover:bg-white/10 hover:text-white'}`} style={currentTool === s.id ? { backgroundColor: accentColor } : {}} title={s.label}>{s.icon}</button>))}
                                                    </div>
                                                )}
                                            </div>
                                            <div className="relative">
                                                <button onClick={() => setIsStickerPickerOpen(!isStickerPickerOpen)} className={`p-1 rounded-lg transition-all ${currentTool === 'sticker' ? 'text-gray-900' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`} style={currentTool === 'sticker' ? { backgroundColor: accentColor } : {}} title="스티커"><span className="text-base leading-none">{currentTool === 'sticker' ? selectedSticker : <Smile size={16} />}</span></button>
                                                {isStickerPickerOpen && (
                                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 bg-gray-800 shadow-2xl border border-white/10 rounded-2xl p-3 grid grid-cols-4 gap-2 w-48 backdrop-blur-xl z-[100]">
                                                        {stickers.map(s => (<button key={s} onClick={() => { setSelectedSticker(s); setTool('sticker'); setIsStickerPickerOpen(false); (window as any).__pagepost_selected_sticker = s; }} className="text-2xl hover:bg-white/10 p-2 rounded-xl transition-all hover:scale-125">{s}</button>))}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} bg-white/5 rounded-xl p-1 border border-white/5 items-center gap-1`}>
                                            <button onClick={() => undoMarkup()} className="p-1 text-gray-400 hover:text-white rounded-lg hover:bg-white/10 transition-all shrink-0" title="실행 취소 (Undo)"><Undo2 size={16} /></button>
                                            {!['eraser', 'select'].includes(currentTool) && (
                                                <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} items-center gap-0.5 bg-white/10 rounded-lg p-0.5 border border-white/5 shadow-inner shrink-0`}>
                                                    <button onClick={() => { const key = currentTool === 'highlight' ? 'highlightWidth' : 'penWidth'; const min = currentTool === 'highlight' ? 5 : 1; const newValue = Math.max(min, (settings as any)[key] - (currentTool === 'highlight' ? 5 : 1)); updateSettings({ [key]: newValue }); if (selectedMarkupId) { const markup = markups.find(m => m.id === selectedMarkupId); if (markup) { updateMarkup(selectedMarkupId, { style: { ...markup.style, strokeWidth: newValue } }); } } }} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/10 transition-all" title="두께 감소"><Minus size={12} /></button>
                                                    <div className="flex flex-col items-center justify-center w-5"><span className="text-[9px] font-bold leading-none" style={{ color: accentColor }}>{currentTool === 'highlight' ? settings.highlightWidth : settings.penWidth}</span></div>
                                                    <button onClick={() => { const key = currentTool === 'highlight' ? 'highlightWidth' : 'penWidth'; const max = currentTool === 'highlight' ? 100 : 20; const newValue = Math.min(max, (settings as any)[key] + (currentTool === 'highlight' ? 5 : 1)); updateSettings({ [key]: newValue }); if (selectedMarkupId) { const markup = markups.find(m => m.id === selectedMarkupId); if (markup) { updateMarkup(selectedMarkupId, { style: { ...markup.style, strokeWidth: newValue } }); } } }} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/10 transition-all" title="두께 증가"><span className="text-xs font-bold">+</span></button>
                                                </div>
                                            )}

                                            {!['eraser', 'select'].includes(currentTool) && (
                                                <div className={`flex ${isVertical ? 'flex-col' : 'flex-row'} items-center gap-0.5 bg-white/10 rounded-lg p-0.5 border border-white/5 shadow-inner shrink-0`}>
                                                    <button onClick={() => setOpacity(Math.max(0.1, settings.markupOpacity - 0.1))} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/10 transition-all" title="투명도 감소"><Minus size={12} /></button>
                                                    <div className="flex flex-col items-center justify-center w-7" title="마크업 투명도"><span className="text-[9px] font-bold leading-none" style={{ color: accentColor }}>{Math.round(settings.markupOpacity * 100)}%</span></div>
                                                    <button onClick={() => setOpacity(Math.min(1.0, settings.markupOpacity + 0.1))} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded-md hover:bg-white/10 transition-all" title="투명도 증가"><span className="text-xs font-bold">+</span></button>
                                                </div>
                                            )}

                                            <div className="flex items-center px-0.5">
                                                <div className="relative w-8 h-8 rounded-full border flex items-center justify-center overflow-hidden transition-transform hover:scale-110 shrink-0" style={{ backgroundColor: currentColor, borderColor: `${accentColor}80` }}>
                                                    <Palette size={14} className="text-white mix-blend-difference pointer-events-none" />
                                                    <input type="color" value={currentColor} onChange={(e) => setColor(e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" title="마크업 색상" />
                                                </div>
                                            </div>
                                            <button onClick={() => { if (confirm('이 페이지의 모든 마킹을 지우시겠습니까?')) clearAllMarkups(); }} className="p-1 text-gray-400 hover:text-red-400 rounded-lg hover:bg-red-500/10 transition-all shrink-0" title="모두 지우기"><Trash2 size={16} /></button>
                                        </div>
                                    </div>
                                )}
                    ...
                    {/* 2. 축소 상태 로고 레이어 (Collapsed Logo Layer) */}
                    <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${!isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-75 pointer-events-none'}`}>
